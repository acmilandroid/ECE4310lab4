...
hWnd=CreateWindow("PLUS","plus program",
		...
		CW_USEDEFAULT,0,400,400,NULL,NULL,hInstance,NULL); //size of window
if (!hWnd)
  return(FALSE);
...
while (GetMessage(&msg,NULL,0,0)) //event loop
  {
...
switch (uMsg)
  ...
  case WM_COMMAND:
    switch (LOWORD(wParam))
      {
	  case ID_PIXELCOLOR_RED:
		  color = 1;
		  break;
	  case ID_PIXELCOLOR_G:
		  color = 2;
		  break;
	  case ID_PIXELCOLOR_BLUE:
		  color = 3;
		  break;
	  ...
	  case ID_FILE_LOAD:
		...
		newImage = (unsigned char *)calloc(ROWS*COLS, 1);
		header[0]=fgetc(fpt);	/* whitespace character after header */
		fread(OriginalImage,1,ROWS*COLS,fpt);
		for (int i = 0; i < ROWS*COLS; i++) { //copies original to new image
			newImage[i] = OriginalImage[i];
		}
		fclose(fpt);
  ...
  case WM_PAINT:
    ...
    return(DefWindowProc(hWnd,uMsg,wParam,lParam));
	break;
  case WM_LBUTTONDOWN:case WM_RBUTTONDOWN: //left and right button click
    return(DefWindowProc(hWnd,uMsg,wParam,lParam));
	break;
  case WM_MOUSEMOVE: //moving mouse
	if (ShowPixelCoords == 1)
	  ...
	  if (xPos >= 0  &&  xPos < COLS  &&  yPos >= 0  &&  yPos < ROWS)
		...
		hDC=GetDC(MainWnd);
		TextOut(hDC,0,0,text,strlen(text));		/* draw text on the window */ //also a printf
...
/* get color */
if (color == 0) {
	CheckMenuItem(hMenu, ID_PIXELCOLOR_RED, MF_UNCHECKED);
	CheckMenuItem(hMenu, ID_PIXELCOLOR_G, MF_UNCHECKED);
	CheckMenuItem(hMenu, ID_PIXELCOLOR_BLUE, MF_UNCHECKED);
}
else if (color == 1) {
	CheckMenuItem(hMenu, ID_PIXELCOLOR_RED, MF_CHECKED);
	CheckMenuItem(hMenu, ID_PIXELCOLOR_G, MF_UNCHECKED);
	CheckMenuItem(hMenu, ID_PIXELCOLOR_BLUE, MF_UNCHECKED);
}
else if (color == 2) {
	CheckMenuItem(hMenu, ID_PIXELCOLOR_RED, MF_UNCHECKED);
	CheckMenuItem(hMenu, ID_PIXELCOLOR_G, MF_CHECKED);
	CheckMenuItem(hMenu, ID_PIXELCOLOR_BLUE, MF_UNCHECKED);
}
else if (color == 3) {
	CheckMenuItem(hMenu, ID_PIXELCOLOR_RED, MF_UNCHECKED);
	CheckMenuItem(hMenu, ID_PIXELCOLOR_G, MF_UNCHECKED);
	CheckMenuItem(hMenu, ID_PIXELCOLOR_BLUE, MF_CHECKED);
}

/* get mode */
if (mode == 1) {
	CheckMenuItem(hMenu, ID_DISPLAY_PLAYMODE, MF_CHECKED);
	CheckMenuItem(hMenu, ID_DISPLAY_STEPMODE, MF_UNCHECKED);
}
else if (mode == 2) {
	CheckMenuItem(hMenu, ID_DISPLAY_PLAYMODE, MF_UNCHECKED);
	CheckMenuItem(hMenu, ID_DISPLAY_STEPMODE, MF_CHECKED);
}
else {
	CheckMenuItem(hMenu, ID_DISPLAY_PLAYMODE, MF_UNCHECKED);
	CheckMenuItem(hMenu, ID_DISPLAY_STEPMODE, MF_UNCHECKED);
